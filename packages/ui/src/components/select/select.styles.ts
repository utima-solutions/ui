...
export const selectDef = {
  item: {
    base: 'relative cur-pointer flex w-full cursor-pointer select-none items-center rounded text-primary outline-none focus:bg-input-border/35 transition-colors focus:text-primary data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
    text: 'relative left-5',
    icon: 'absolute left-2 flex size-3.5 items-center justify-center',
  },
  label: 'py-1.5 pl-2 font-semibold text-primary',
  separator: '-mx-1 my-1 h-px bg-muted',
  trigger:
    'flex w-full transition-colors items-center justify-between rounded-md border border-input-border focus-visible:border-foreground hover:border-foreground bg-background px-3 py-2 text-sm text-primary ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-border focus-visible:border-foreground placeholder:text-muted-foreground focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
  content: {
