...
export const selectDef = twOverrides(
  {
    item: {
      base: 'transition-all relative cur-pointer flex w-full cursor-pointer select-none items-center rounded text-input-fg outline-none focus:bg-accent focus:text-accent-fg data-[state=checked]:bg-accent data-[disabled]:pointer-events-none data-[disabled]:opacity-50',
      text: 'relative',
    ...
    trigger: `${globalDef.ring} text-left bg-input ring-input-border flex w-full transition-colors items-center justify-between rounded-md border border-input-border hover:border-input-fg px-3 py-2 text-sm text-input-fg placeholder:text-placeholder focus:outline-none disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1`,
    content: {
